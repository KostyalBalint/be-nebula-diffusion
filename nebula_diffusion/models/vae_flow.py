...
class FlowVAE(Module):
    ...
    def __init__(self, args):
        super().__init__()
        self.args = args
        size = args.model_size if hasattr(args, 'model_size') else 'original'
        self.encoder = PointNetEncoder(args.latent_dim, size=size)
        self.flow = build_latent_flow(args)
        self.diffusion = DiffusionPoint(
            net = PointwiseNet(point_dim=3, context_dim=args.latent_dim + args.latent_text_dim, residual=args.residual, size=size),
    ...
    def sample(self, w, encoded_text, num_points, flexibility, truncate_std=None, stream_diffusion=False):
        batch_size, _ = w.size()
        ...
        conditioned_z = torch.cat((z, encoded_text), 1)
        samples = self.diffusion.sample(num_points, context=conditioned_z, flexibility=flexibility, stream_diffusion=stream_diffusion)
        return samples
